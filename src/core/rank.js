// Turns scored posts into the two ranked shortlists that become the brief's spine.
export function buildShortlists(posts) {
  const relevant = posts.filter((p) => p.flags.relevant);
  const excluded = posts.filter((p) => !p.flags.relevant); // off-topic — kept for transparency
